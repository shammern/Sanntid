...
class Resource(T)
{
    ...
    T allocate(int priority)
    {
        ...
        if (busy)
        {
            // Resource is in use, so record that one more task of this priority is waiting.
            numWaiting[priority]++;
            // Release the outer lock before waiting.
            mtx.notify();
            // Wait on the semaphore for the given priority.
            sems[priority].wait();
            numWaiting[priority]--;
        }
        // Mark the resource as busy and release the lock.
        busy = true;
        mtx.notify();
        return value;
    }
    ...
    void deallocate(T v)
    {
        // Acquire the outer lock.
        mtx.wait();
        // Update the resource value and mark it as free.
        value = v;
        busy = false;
        // First, check if any high-priority tasks are waiting.
        if (numWaiting[1] > 0)
        {
            sems[1].notify();
        }
        // Otherwise, if there are low-priority tasks waiting, signal one.
        else if (numWaiting[0] > 0)
        {
            sems[0].notify();
        }
        // If no tasks are waiting, simply release the outer lock.
        else
        {
            mtx.notify();
        }
