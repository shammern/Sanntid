...
- Releasing multiple users:  
  Test: Multiple users of different priorities show up at the same time.  (even > odd)
...
All three mechanisms allow tasks to wait and resume based on shared resource availability.
POSIX condition variables are powerful but require manual locking and signaling, which is error-prone.
Java monitors simplify things with built-in synchronization, but still require careful use of wait() and notify().
Ada protected objects use guards and entries to handle waiting automatically and safely.
This reduces complexity and avoids many common synchronization bugs seen in other models.
...
I am most confident in Ada protected object solutions because they are declarative and enforced by the compiler.
Protected entries only run when their guards are true, eliminating the need for manual checks.
This removes bugs like missed signals or incorrect locking that often appear in condition variable code.
Code written this way is easier to understand and proves correct under more conditions.
That improves quality by shifting responsibility from the programmer to the language and runtime.
...
To support N priorities, you could define separate entries or use a priority queue inside the resource.
Each lower-priority request would need to check that no higher-priority tasks are waiting.
This is possible in Ada but gets harder to manage as the number of priorities increases.
The solution becomes more complex and harder to read, reducing the code’s elegance and maintainability.
It highlights that some synchronization mechanisms scale poorly when requirements grow beyond simple use cases.
...
Using getValue() on a semaphore usually leads to incorrect assumptions and race conditions.
Its value can change immediately after reading, making it unreliable for decision-making.
Programs that rely on it often break under concurrency because the state isn’t guaranteed.
Instead, it’s better to block and wait using proper synchronization than to poll for availability.
Hiding getValue() in libraries is smart because it prevents misuse and encourages safer patterns.
...
For this task, I prefer Ada protected objects because they make correctness easier to achieve.
The syntax is clean, and the behavior is predictable thanks to entry guards and built-in queuing.
They’re ideal in real-time and safety-critical environments where predictability is essential.
Java monitors are fine for general-purpose use but require more discipline.
POSIX condition variables offer control but trade off safety, making them my last choice.
